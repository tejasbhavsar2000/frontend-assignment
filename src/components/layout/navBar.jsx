...
const NavBar = () => {
  ...
  return (
    <div className="w-full md:w-[15.125rem] md:max-h-screen sticky top-0 md:justify-center md:border-r md:border-r-[#E9EAEB]  flex  flex-col bg-white  ">
      <div
        className={`flex w-full bg-white z-50 md:p-5  justify-between p-4 md:shadow-none   ${
          active ? "" : " shadow-[0_0_0_1px_#E9EAEB]"
        }`}
      >
      ...
      <div className="flex-1  relative font-[Inter]">
        <div
          className={`md:h-full md:static md:translate-y-0 md:gap-6 flex flex-col absolute z-20  w-full   bg-white  p-4 text-base font-semibold gap-4 transition-all  ${
        ...
        >
          ...
          <nav className="flex flex-1 flex-col md:justify-between font-semibolds  text-[#414651]">
            <div className="flex flex-col md:gap-1">
              {NavItems.map((item, id) => {
                return (
                  <NavItem
                    className={`md:${id == 5 || id == 6 ? "hidden" : ""}`}
      ...
      <div
        className={`absolute h-screen w-screen bg-[#6C6C6C4D] z-10  ${
          active ? "" : "hidden"
        }`}
      />
    </div>
